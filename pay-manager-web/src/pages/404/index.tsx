...
export default () => {
  const {  isLogin,  _logout } = useModel('user');
  const hasPageAuth = common.permissionUtils.hasAsyncRouter()
  // 按钮文字
  const getBtnText = () => {
    return hasPageAuth ? '返回' : '重新登录';
  };

  const handleGoBack = () => {
    // 如果未登录或者无路由
    if (!isLogin || !hasPageAuth) {
      _logout()
    }
  ...
  return (
    <div className="page404-wrapper">
      ...
      <div className="wrapper-opt">
        <WithButton auth type="primary" size="large" onClick={handleGoBack}>
          {getBtnText()}
        </WithButton>
      </div>
    </div>
