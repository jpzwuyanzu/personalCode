...
import { cloneDeep, isEmpty , difference} from 'lodash';
...
export default (props: any) => {
  ...
  useAsyncEffect(async () => {
    if (visible) {
      setLoading(true);
      const data = await services.system.account.getUserRouterMenu({ userId: current.userId })
        .finally(() => setLoading(false));


      if (utils.types(data) === 'array') {
        setRoleTree(data as any);
        setCheckedIds([]);
        getCheckedKeys(data as any, []);
      }
    }
  }, [visible]);
