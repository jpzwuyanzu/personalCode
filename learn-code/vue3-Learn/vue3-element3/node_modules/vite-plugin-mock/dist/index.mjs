  ...
  const middleware = async (req, res, next) => {
    ...
    if (matchRequest) {
      ...
      if (isFunction(rawResponse)) {
        ...
        const mockResponse = isFunction(response) ? response({body, query, headers: req.headers}) : response;
...
async function getMockConfig(opt) {
  ...
  const mockFiles = fg.sync(`**/*.${supportTs ? "ts" : "js"}`, {
    cwd: absMockPath
  }).filter((item) => {
    if (!ignore) {
      return true;
...
function viteMockServe(opt) {
  ...
  const defaultEnter = normalizePath(path2.resolve(process.cwd(), `src/main.${supportTs ? "ts" : "js"}`));
